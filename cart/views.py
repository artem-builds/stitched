...
class   CartModalView(CartMixin, View):
    def ger (self, request):
        ...
        context = {
            'cart': cart,
            'cart_items': cart.items.select_related('product', 'product_size__size').order_by('-addedAt')
        }
...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if product_size.stock < quantity:
            return JsonResponse({
                'error': 'Only {product_size.stock} items available', 
                }, status=400)
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error': 'Cannon add {quantity} items to {product_size.size.stock - existing_item.quantity} more available.',
                    }, status=400)
...
class UpdateCartItemView(CartMixin, View):
    ...
    def post(self, request, item_id):
        ...
        context = {
            'cart': cart,
            'cart_item': cart.items.select_related('product', 'product_size__size').order_by('-addedAt')
        }
